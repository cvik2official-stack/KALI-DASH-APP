<script setup lang="ts">
import { ref, onMounted } from 'vue';
import { Button } from '@/components/ui/button';
import { loadCsvData } from '@/lib/csvUtils';
import CsvTable from '@/components/CsvTable.vue';
import { showInfoToast, showErrorToast, showSuccessToast } from '@/lib/toast';
import {
  Sheet,
  SheetTrigger,
  SheetContent,
} from '@/components/ui/sheet';
import { Menu } from 'lucide-vue-next';
import MenuPanel from '@/components/MenuPanel.vue';
import type { CsvRow } from '@/types';
import OrderSelectionSheet from '@/components/OrderSelectionSheet.vue'; // Import the new component

const csvData = ref<CsvRow[]>([]);
const isLoading = ref(true);
const error = ref<string | null>(null);
const isMenuOpen = ref(false);

// New state for the order selection sheet
const isOrderSheetOpen = ref(false);
const orderSessionData = ref<CsvRow[]>([]);

onMounted(async () => {
  showInfoToast('Loading CSV data...');
  try {
    const data = await loadCsvData('/data/grocery-list.csv');
    csvData.value = data as CsvRow[]; // Directly assign data, assuming NAME is present
    showSuccessToast('CSV data loaded successfully!');
  } catch (e: any) {
    // Ensure e.message is treated as a string
    error.value = (e instanceof Error ? e.message : String(e)) || 'Failed to load CSV data.';
    showErrorToast(error.value);
  } finally {
    isLoading.value = false;
  }
});

const handleCreateOrder = () => {
  if (csvData.value.length === 0) {
    showInfoToast('No data available to create an order.');
    return;
  }
  // Create a deep copy of the current CSV data for the order session
  orderSessionData.value = JSON.parse(JSON.stringify(csvData.value));
  isOrderSheetOpen.value = true;
};
</script>

<template>
  <div class="flex flex-col min-h-full p-4">
    <div class="w-full flex items-center justify-between mb-6">
      <Sheet v-model:open="isMenuOpen">
        <SheetTrigger as-child>
          <Button variant="outline" size="icon">
            <Menu class="h-4 w-4" />
            <span class="sr-only">Open menu</span>
          </Button>
        </SheetTrigger>
        <SheetContent side="left" class="p-0 w-64">
          <MenuPanel />
        </SheetContent>
      </Sheet>

      <h1 class="text-2xl font-bold">Admin Dashboard</h1>

      <Button @click="handleCreateOrder">Create Order</Button>
    </div>
    
    <div class="flex-1 flex flex-col mt-4">
      <div v-if="isLoading" class="text-xl text-gray-600 dark:text-gray-300 text-center">Loading data...</div>
      <div v-else-if="error" class="text-xl text-red-600 dark:text-red-400 text-center">Error: {{ error }}</div>
      <div v-else-if="csvData.length > 0" class="w-full max-w-4xl mx-auto flex-1">
        <CsvTable
          :initial-data="csvData"
          :is-order-mode="false"
          :cart-items="[]"
        />
      </div>
      <div v-else class="text-xl text-gray-600 dark:text-gray-300 text-center">No CSV data available.</div>
    </div>

    <!-- The new OrderSelectionSheet component -->
    <OrderSelectionSheet
      v-model:open="isOrderSheetOpen"
      :initial-order-data="orderSessionData"
    />
  </div>
</template>

<style scoped>
/* No specific styles needed here, Tailwind handles it */
</style>